  ...
  return (
    <div className="space-y-3">
      {epics.map((epic) => {
        ...
        return (
          <div key={epic.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            {/* Epic header */}
            <div
              className="group flex items-center gap-3 p-4 cursor-pointer hover:bg-slate-50 transition-colors"
              style={{ borderLeft: `4px solid ${epic.color}` }}
            >
              <button
                type="button"
                onClick={() => toggleEpic(epic.id)}
                className="shrink-0 rounded p-0.5 text-slate-400 hover:bg-slate-200 hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/40"
              ...
              <div className="flex-1 min-w-0" onClick={() => setSelectedEpicId(epic.id)}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-base font-semibold text-slate-800 truncate">{epic.title}</h3>
            ...
            {isExpanded && (
              <div className="border-t border-slate-100 bg-slate-50/50 px-4 py-3 space-y-2">
                {stories.length === 0 ? (
                  <p className="text-sm text-slate-400 italic py-2">No stories in this epic yet.</p>
                ) : (
                  stories.map((story) => {
                    ...
                    return (
                      <div key={story.id} className="rounded-lg border border-slate-200 bg-white overflow-hidden">
                        {/* Story header */}
                        <div className="group flex items-center gap-3 p-3 cursor-pointer hover:bg-slate-50 transition-colors">
                          <button
                            type="button"
                          ...
                          <div className="flex-1 min-w-0" onClick={() => setSelectedStoryId(story.id)}>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <p className="text-sm font-semibold text-slate-700 truncate">{story.title}</p>
                          ...
                          <div className="shrink-0 flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-500">{tasks.length} {tasks.length === 1 ? 'task' : 'tasks'}</span>
                          </div>
                        </div>
                        ...
                        {storyExpanded && (
                          <div className="border-t border-slate-100 bg-slate-50/50 px-3 py-2 space-y-1.5">
                            {tasks.length === 0 ? (
                              <p className="text-xs text-slate-400 italic py-1">No tasks in this story yet.</p>
                            ) : (
                              tasks.map((task) => {
